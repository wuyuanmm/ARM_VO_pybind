...
void tracker::track(const Mat &img_1, const Mat &img_2, vector<Point2f>& points1, vector<Point2f>& points2)
{
...
#else
	vector<Point2f> tmp_pt1, tmp_pt2;
	for (int i = indexCorrection; i<status.size(); i++)
	{
		// pt = points2[i- indexCorrection];
		...
		if ((status[i] == 0) || (pt.x<0) || (pt.y<0) || (pt.x>img_1.cols) || (pt.y>img_1.rows))
		{
			//points1.erase(points1.begin()+i- indexCorrection);
		...
		{
			tmp_pt1.push_back(points1[i]);
			tmp_pt2.push_back(points2[i]);
		}
		///wuyuan
	}
	...
	points1.assign(tmp_pt1.begin(), tmp_pt1.end());
	points2.assign(tmp_pt2.begin(), tmp_pt2.end());
#endif
}
...
void tracker::calcSharrDeriv(const Mat& src, Mat& dst)
{
    ...
    for( y = 0; y < rows; y++ )
    {
        ...
        for( ; x <= colsn - 8; x += 8 )
        {
			...
			int16x8_t q0 = vld1q_s16((const int16_t*)&(trow0[x + cn]));
			if (x == 0)
				q1 = vld1q_s16((const int16_t*)(_tempBuf));
			else
				q1 = vld1q_s16((const int16_t*)&(trow0[x - cn]));
			int16x8_t q2 = vld1q_s16((const int16_t*)&trow1[x + cn]);
			//int16x8_t q3 = vld1q_s16((const int16_t*)&trow1[x-cn]);

			if (x == 0)
				q3 = vld1q_s16((const int16_t*)(trow0 + delta - cn));
			else
				q3 = vld1q_s16((const int16_t*)&(trow1[x - cn]));
